...
<style>
	.link {
		...
		background-color: rgb(from var(--ds-surface-raised) r g b / 0.6);
	...
	@media (hover: hover) and (pointer: fine) {
		.link:hover {
			/* vorher ungültige Syntax (ohne rgb(from …)) — Hover feuerte nie */
			background-color: rgb(from var(--ds-surface-raised) r g b / 0.2);
		}
	}
