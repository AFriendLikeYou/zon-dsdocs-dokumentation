	...
	for (let page = 0; page < 4; page++) {
		...
		if (hit)
			return {
				...
				user_handle: hit.user?.handle || null
			};
...
async function main() {
	...
	if (!args.file || !args.node) {
		console.error(
			'Usage: node blame-node.mjs --file <KEY_OR_URL> --node <ID> (--property <KEY> | --child <ID>) [--start <VID|current>] [--max N] [--no-autosaves]'
		);
		process.exit(1);
	}
	...
	if (!startNode) {
		console.error(
			`Node ${nodeId} not found at start version "${startVer}". Verify --node and --start.`
		);
		process.exit(1);
	}
	if (!targetExists(startNode, target)) {
		console.error(
			`Target was not found at start version "${startVer}". The search requires the target to exist at start (you're asking when it was INTRODUCED). To track something that was REMOVED, pick a --start where it still existed.`
		);
		process.exit(1);
	}
	...
	const versions = await collectCandidateVersions(
		fileKey,
		resolvedStart,
		lookback,
		includeAutosaves
	...
	if (oldestExistsIdx === -1) {
		...
		certainty =
			introduced.user_handle === 'Figma'
				? 'system_attributed'
				: introduced.user_handle
					? 'exact'
					: 'metadata_unavailable';
		if (certainty === 'metadata_unavailable')
			notes.push(
				"Introduced at the start version itself, but its author metadata wasn't found within the versions-list lookback. The introduction is real; the user just isn't attributable from REST data alone."
			);
	} else {
		...
		if (oldestExistsIdx === versions.length - 1) {
			...
			notes.push(
				`Target also exists at the oldest scanned version (${v.id}). The true introduction is older than the search range — increase --max (currently ${lookback}).`
			);
		} else if (introduced.user_handle === 'Figma') {
			certainty = 'system_attributed';
			notes.push(
				"The introduction version was a system autosave (user_handle='Figma'). Re-run with --no-autosaves to snap to the nearest LABELED version containing the change."
			);
		} else {
			certainty = 'exact';
		}
	...
	notes.push(
		"Binary search assumes the target's existence is monotonic (added once, never removed). If it was added, removed, then re-added, the reported introduction point may differ from the original."
	);
	...
	const result = {
		...
		target: args.property
			? { type: 'component_property', name: args.property }
			: { type: 'child_node', node_id: args.child },
...
main().catch((e) => {
	let hint = '';
	if (e.status === 403)
		hint = " Hint: ensure your PAT has both 'File content: Read' and 'File versions: Read'.";
