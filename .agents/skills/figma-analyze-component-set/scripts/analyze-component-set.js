...
if (node.type !== 'COMPONENT_SET') {
	throw new Error(
		'Node is not a COMPONENT_SET (got ' +
			node.type +
			'). Pass the parent set, not an individual variant.'
	);
}
...
try {
	...
	for (const v of allVars) {
		varNameMap[v.id] = {
			name: v.name,
			collection: collMap[v.variableCollectionId] || null,
			type: v.resolvedType
		};
	}
} catch (e) {
	/* variables may be unavailable; fall back to hex */
}
...
async function ensureVar(id) {
	...
	try {
		...
		/* unreachable target — falls back to raw id */
	}
}
async function preResolveBoundVars(n) {
	...
	if ('children' in n && n.children) {
		for (const c of n.children) await preResolveBoundVars(c);
	}
}
	...
	if (mainChild) {
		...
		try {
			...
			if (!sig.fillToken && child.fills && child.fills.length > 0 && child.fills[0].color)
				sig.fillHex = toHex(child.fills[0].color);
			if (!sig.strokeToken && child.strokes && child.strokes.length > 0 && child.strokes[0].color)
				sig.strokeHex = toHex(child.strokes[0].color);
				...
				if (textChild.type === 'TEXT') {
					try {
						...
						if (
							!sig.textToken &&
							textChild.fills &&
							textChild.fills.length > 0 &&
							textChild.fills[0].color
						)
							sig.textHex = toHex(textChild.fills[0].color);
					} catch (e) {}
					break;
				}
			}
...
for (const v of variants) {
	if (
		v.name.indexOf('state=default') !== -1 &&
		(sizeAxis ? v.name.indexOf(sizeAxis + '=') !== -1 : true)
	) {
...
for (const variant of variants) {
	...
	if (defaultSig && variant.id !== (defaultVariant ? defaultVariant.id : null)) {
		...
		if (sig.strokeToken !== defaultSig.strokeToken)
			diff.strokeToken = sig.strokeToken || sig.strokeHex;
		...
		if (JSON.stringify(sig.effects) !== JSON.stringify(defaultSig.effects))
			diff.effects = sig.effects;
