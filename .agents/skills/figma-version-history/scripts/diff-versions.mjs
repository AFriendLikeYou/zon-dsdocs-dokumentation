...
async function getJson(url) {
	...
	if (!resp.ok) {
		...
		let hint = '';
		if (resp.status === 403)
			hint = " (PAT likely missing 'File versions: Read' / 'File content: Read')";
		else if (resp.status === 404)
			hint = " (version_id may have been pruned by retention, or doesn't belong to this file)";
...
function diffNode(fromNode, toNode, mode = 'standard') {
	...
	if (fromNode.type === 'COMPONENT_SET' || toNode.type === 'COMPONENT_SET') {
		...
		if (s.added || s.removed || s.type_changed || s.default_changed)
...
async function main() {
	...
	if (!args.file || !args.from) {
		console.error(
			'Usage: node diff-versions.mjs --file <KEY_OR_URL> --from <VID> [--to <VID|current>] [--components id1,id2] [--mode summary|standard|detailed]'
		);
		process.exit(1);
	}
	...
	const componentIds = args.components
		? args.components
				.split(',')
				.map((s) => s.trim())
				.filter(Boolean)
	...
	if (componentIds.length === 0) {
		notes.push(
			'Only page-structure diff returned. Pass --components <ids> for per-component analysis.'
		);
	}
	notes.push(
