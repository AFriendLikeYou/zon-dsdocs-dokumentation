...
> Kurzfassung: Aus einer in Figma auf _Ready for dev_ markierten Component wird
...
öffnen") + **Tabs** _Design / Develop / Barrierefreiheit / Specs_ über die
...
2. **Global/Basis** — `static/global.css`: Reset, Box-Sizing, Fonts und die
   **Prosa-/Dokument-Typografie für `<main>`** (h1–h5, p, a, Listen, `pre`).
   Das ist _Seiten_-Styling, kein Komponenten-Styling. Dazu eine **kleine,
...
**konsumiert** die Utility; die Styles werden _nicht_ in die Komponente kopiert
...
1. **Content/Spec-Split + GitHub-Stift** — _umgesetzt_ (ADR-008/009).
2. **PR-Preview-Workflow** dokumentieren + `CONTRIBUTING.md` (Vercel-Preview pro
   PR als gemeinsame Review-Fläche; Merge = Veröffentlichen). _Geplant._
...
- 🎨 **Designer** → Figma (_Ready for dev_) + `content.ts` über den Stift +
