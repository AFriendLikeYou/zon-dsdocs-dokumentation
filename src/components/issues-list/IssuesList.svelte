	...
	<div class="faq__inner">
		{#each Array.from(groupedIssues()) as [category, items]}
			<div class="accordion">
				...
				<div
					id={`content-${category}`}
					class="accordion__content faq__content {openCategories[category] ? 'open' : ''}"
				>
					{#each items as issue}
						<article class="issue">
...
<style>
	...
	.issue {
		margin-bottom: 1rem;
		padding-left: 1.5rem;
		border-left: 2px solid var(--z-ds-color-border-70);
	}

	:global(mark) {
		border: 1px solid var(--z-ds-color-focus-100);
	...
	.faq__section .accordion__header {
		...
		text-align: left;
		cursor: pointer;
		color: var(--z-ds-general-color-black-100);
	}
