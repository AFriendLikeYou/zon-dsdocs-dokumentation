...
<div class="dodont">
	...
	<div class="dodont-card__bar" style="--bar-color: {type === 'do' ? 'green' : 'red'}"></div>

	<div class="dodont-card__caption dodont-card__caption--{type.toLowerCase()}">
		<div class="dodont-card__title" style="color: {type === 'do' ? 'green' : 'red'}">
			{type === 'do' ? 'Do' : "Don't"}
		</div>
		{caption}
	</div>
...
<style>
	...
	.dodont-card__content {
		flex: 1;
		aspect-ratio: 16/9;
		background-image: radial-gradient(var(--z-ds-color-border-70) 1px, transparent 1px);
